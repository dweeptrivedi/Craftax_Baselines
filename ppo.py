...
def make_train(config):
    ...
    # ``automata_rl.embedding_setup.build_embedding_stack``. This sits
    # between the inner env (or task env) and ``LogWrapper`` so that
    # ``LogWrapper`` records any reward shaping in the episode return.
    # The augmented obs is a simple flat concat (UVFA-style); the policy
    # network is the upstream ``ActorCritic`` unchanged.
