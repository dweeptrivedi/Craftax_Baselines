#!/usr/bin/env bash
# Train vanilla PPO on Craftax-Symbolic-v1.
#
# Usage:
#   ./train_ppo.sh                   # baseline (default Craftax reward)
#   ./train_ppo.sh collect_wood      # task: reward fires only on `collect_wood`
#
...
cd "$(dirname "$0")"
echo "==> Running: python ppo.py ${ARGS[*]}"
exec python ppo.py "${ARGS[@]}"
