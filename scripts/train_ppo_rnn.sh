#!/usr/bin/env bash
# Train PPO with an RNN policy on Craftax-Symbolic-v1.
#
# Usage:
#   ./train_ppo_rnn.sh                   # baseline (default Craftax reward)
#   ./train_ppo_rnn.sh collect_wood      # task: reward fires only on `collect_wood`
#
...
cd "$(dirname "$0")"
echo "==> Running: python ppo_rnn.py ${ARGS[*]}"
exec python ppo_rnn.py "${ARGS[@]}"
